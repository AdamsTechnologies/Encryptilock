...
void main() {
  group('DartSqlite Tests', () {
    ...
    test('Insert and Query a single row', () {
      ...
      // Insert
      db.insert('people', {'id': 'abc', 'name': 'Alice', 'age': 30});

      // Query
      final rows = db.query('SELECT * FROM people WHERE id=?', ['abc']);
    ...
    test('Update existing rows', () {
      ...
      // Insert a row
      db.insert('books', {'id': 'b1', 'title': 'Old Title', 'author': 'Unknown'});
      // Update the row
      db.update(
        'books',
        {'title': 'New Title', 'author': 'John Doe'},
        whereClause: 'id=?',
        whereArgs: ['b1'],
      );

      // Verify
      final rows = db.query('SELECT title, author FROM books WHERE id=?', ['b1']);
      expect(rows.length, 1);
    ...
    test('Upsert a row (ON CONFLICT ...)', () {
      ...
      // Insert or upsert
      db.upsert('notes', {'id': 'n1', 'content': 'Hello World'}, ['id']);
      // Check inserted
      var rows = db.query('SELECT * FROM notes WHERE id=?', ['n1']);
      expect(rows.length, 1);
      expect(rows.first['content'], 'Hello World');

      // Upsert again with new content
      db.upsert('notes', {'id': 'n1', 'content': 'Updated Content'}, ['id']);
      // Check updated
      rows = db.query('SELECT * FROM notes WHERE id=?', ['n1']);
      expect(rows.length, 1);
